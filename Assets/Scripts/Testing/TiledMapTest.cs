...
public class TiledMapTest : MonoBehaviour {
	...
	void Start () {
		...
		mapData = MapDataTest.buildDefaultMap ();
		width = mapData.tiles.GetLength(0);
		height = mapData.tiles.GetLength(1);
		depth = mapData.tiles.GetLength(2);

		buildWorldSpace ();
	}

	private void buildWorldSpace() {
		BuildMesh ();
	}
	...
	public Tile[,] getNonaTile(int x, int y)
	{
		...
		returnTiles[0, 0] = mapData.getTile(x - 1, y - 1, 5);
		returnTiles[1, 0] = mapData.getTile(x, y - 1, 5);
		returnTiles[2, 0] = mapData.getTile(x + 1, y - 1, 5);
		returnTiles[0, 1] = mapData.getTile(x - 1, y, 5);
		returnTiles[1, 1] = mapData.getTile(x, y, 5);
		returnTiles[2, 1] = mapData.getTile(x + 1, y, 5);
		returnTiles[0, 2] = mapData.getTile(x, y + 1, 5);
		returnTiles[1, 2] = mapData.getTile(x - 1, y + 1, 5);
		returnTiles[2, 2] = mapData.getTile(x + 1, y + 1, 5);
	...
	public void RpcDigFromWorldSpace(Vector2 location) {
		...
		mapData.digWall ((int)mapCoords.x, (int)mapCoords.y, 5);
	...
	public void loadMapData(MapDataTest data) {
		mapData = data;
		DrawMap ();
	}
}
