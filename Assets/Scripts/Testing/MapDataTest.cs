...
public class MapDataTest {
	...
	public MapDataTest(int length, int width, int height) {

		tiles = new Tile[width, length, height];

		for (int i = 0; i < width; i++) {
			for (int j = 0; j < length; j++) {
				for (int k = 0; k < height; height++) {
	...
	public static MapDataTest buildDefaultMap() {
		MapDataTest returnMap = new MapDataTest (40, 40, 10);

		returnMap.tiles [3,3,5] = new Dirt ();
		returnMap.tiles [3,4,5] = new Dirt ();
		returnMap.tiles [3,5,5] = new Dirt ();
		returnMap.tiles [4,3,5] = new Dirt ();
		returnMap.tiles [4,4,5] = new Dirt ();
		returnMap.tiles [4,5,5] = new Dirt ();
		returnMap.tiles [5,3,5] = new Dirt ();
		returnMap.tiles [5,4,5] = new Dirt ();
		returnMap.tiles [5,5,5] = new Dirt ();

		returnMap.tiles[2, 2,5] = new Wall(WallType.Angle_In_UL);
		returnMap.tiles[3, 2,5] = new Wall(WallType.Up);
		returnMap.tiles[4, 2,5] = new Wall(WallType.Up);
		returnMap.tiles[5, 2,5] = new Wall(WallType.Up);
		returnMap.tiles[6, 2,5] = new Wall(WallType.Angle_In_UR);

		returnMap.tiles[2, 3,5] = new Wall(WallType.Left);
		returnMap.tiles[2, 4,5] = new Wall(WallType.Left);
		returnMap.tiles[2, 5,5] = new Wall(WallType.Left);
		returnMap.tiles[2, 6,5] = new Wall(WallType.Angle_In_DL);

		returnMap.tiles[3, 6,5] = new Wall(WallType.Down);
		returnMap.tiles[4, 6,5] = new Wall(WallType.Down);
		returnMap.tiles[5, 6,5] = new Wall(WallType.Down);
		returnMap.tiles[6, 6,5] = new Wall(WallType.Angle_In_DR);

		returnMap.tiles[6, 3,5] = new Wall(WallType.Right);
		returnMap.tiles[6, 4,5] = new Wall(WallType.Right);
		returnMap.tiles[6, 5,5] = new Wall(WallType.Right);
	...
	public void digWall(int x, int y, int depth)
	{
		...
		setTile(x, y, 5, new Dirt());

		//update all surrounding walls
		updateWall(x - 1, y - 1, 5);
		updateWall(x, y - 1, 5);
		updateWall(x + 1, y - 1, 5);
		updateWall(x - 1, y, 5);
		updateWall(x + 1, y, 5);
